...
public class Client extends Connection implements EndPoint
{
	 ...
	 *            Two (using only TCP) or four (using both TCP and UDP) buffers
	...
	/**
	 * Calls {@link #connect(int, InetAddress, int) connect} with the values
	...
	private void broadcast(int udpPort, DatagramSocket socket) throws IOException
	{
		...
		//mine added to troubleshoot -did nothing
		info("kryonet", "broadcast: setting broadcast to true");
//		socket.setBroadcast(true);
		
		//all that was done before was the broadcastsocket was created
		//so- going to try what they had from another post
//		info("kryonet", "broadcast: setting socket to a channel socket");
//		DatagramChannel channel = DatagramChannel.open();
//		socket = channel.socket();


		info("kryonet", "broadcast: creating databuffer");
		ByteBuffer dataBuffer = ByteBuffer.allocate(64);
		info("kryonet", "broadcast: writing to serialization");
		serialization.write(null, dataBuffer, new DiscoverHost());
		info("kryonet", "broadcast: flipping data buffer");
		dataBuffer.flip();
		info("kryonet", "broadcast: creating new byte array");
		byte[] data = new byte[dataBuffer.limit()];
		info("kryonet", "broadcast: getting the data");
		dataBuffer.get(data);
		info("kryonet", "broadcast: about to look at network interfaces");
		for (NetworkInterface iface : Collections.list(NetworkInterface.getNetworkInterfaces()))
		{
			info("kryonet", "broadcast: looking at a network interface");
			for (InetAddress address : Collections.list(iface.getInetAddresses()))
			{
				info("kryonet", "broadcast: looking at InetAddress");
				// Java 1.5 doesn't support getting the subnet mask, so try the two most common.
				byte[] ip = address.getAddress();
				ip[3] = -1; // 255.255.255.0
				
				info("kryonet", "broadcast: getting inet address");
				InetAddress inet_address = InetAddress.getByAddress(ip);
				
				info("kryonet", "broadcast: creating datagram package");
				DatagramPacket data_packet =new DatagramPacket(data, data.length, inet_address, udpPort);
				
				//added to hopefully try multiple times
				try
				{
				
					info("kryonet", "broadcast: going to send 1st data packet"); 
					socket.send(data_packet);
					ip[2] = -1; // 255.255.0.0
					
					info("kryonet", "broadcast: getting inet address again ");
					inet_address = InetAddress.getByAddress(ip);
					info("kryonet", "broadcast: going to send 2nd data packet");
					socket.send(new DatagramPacket(data, data.length, inet_address, udpPort));
				}
				catch(SocketException err)
				{
					info("kryonet", "broadcast: COULD NOT send socket!!!!!"+err);
				}
			}
		}
		if (DEBUG)
	...
	{
		info("kryonet", "discoverHost: setting socket to null");
		DatagramSocket socket = null;
		try
		{
			info("kryonet", "discoverHost: creating the socket");
			socket = new DatagramSocket();
			info("kryonet", "discoverHost: aboput to broadcast");
			broadcast(udpPort, socket);
			info("kryonet", "discoverHost: setting the timeout");
			socket.setSoTimeout(timeoutMillis);
			
			info("kryonet", "discoverHost: creating the packet");
			DatagramPacket packet = new DatagramPacket(new byte[0], 0);
			try
			{
				info("kryonet", "discoverHost: Trying to recieve packet");
